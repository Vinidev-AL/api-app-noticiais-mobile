...
export class NoticiasService {
    ...
    listAll() {
        return this.db.select().from(noticias).orderBy(desc(noticias.dataCriacao)).all();
    }

    getById(id: string, user?: CurrentUserPayload) {
        const noticia = this.db.select().from(noticias).where(eq(noticias.id, id)).get();
        if (!noticia) {
    ...
    create(dto: CreateNoticiaDto, autorId: string) {
        const id = randomUUID();
        const now = new Date();
        this.db.insert(noticias).values({
            id,
            titulo: dto.titulo,
    ...
    linkTag(noticiaId: string, tagId: string, user: CurrentUserPayload) {
        ...
        if (
            user.role === Role.AUTOR && noticia.autorId !== user.userId
        ) {
            throw new ForbiddenException('Acesso negado.');
        }
        ...
        this.db.insert(noticiaTags).values({ noticiaId, tagId }).onConflictDoNothing().run();
        return { linked: true };
    }
    ...
    unlinkTag(noticiaId: string, tagId: string, user: CurrentUserPayload) {
        ...
        this.db.delete(noticiaTags).where(and(eq(noticiaTags.noticiaId, noticiaId), eq(noticiaTags.tagId, tagId))).run();
